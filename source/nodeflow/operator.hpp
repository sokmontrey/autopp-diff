 ...
 *      Constructor argument can be individual individual argument form:
 * OPERATOR_NODE_NAME a(&a, &b, ...); OR list argument form: OPERATOR_NODE_NAME
 * a{&a, &b, ...}; The number of argument is depend on the type of operator node
 */

/**
 * Below is a template for creating a custom operator node, extensions of
OperatorNode with different compute and derivative function
 *
class FunctionName: public OperatorNode<[number of input]>{
    ...
    Eigen::MatrixXd derivative(size_t input_wrt_index) override {
        //MUST: return a matrix as the result to the taking the partial
derivative on the function with respect to this->inputs[input_wrt_index]
        //MUST: use this->outer_derivative according to chain rule
        //EXAMPLE: return this->outer_derivative;
        //      return outer_derivative directly since the partial derivative
with respect to any of the input
        //      is 1 matrix. And 1 * outer_deivative is the outer_derivative
itself.
    }
};
 *
 * The number of rows and cols of a Node is very important
 *      OperatorNode will automatically set the rows and cols to the rows and
cols of the first input node.
 *      If an OperatorNode has a different number of rows and cols, for example
 ...
 *
 * DERIVATIVE: Jacobian matrix is multiplied directly with outer derivative
 * directly to get sum of partial derivative ∂a = b.T * outer_derivative ∂b =
 * outer_derivative * a.T
 *
 * @params: a: Node*: pointer to first input Node
 *          b: Node*: pointer to second input Node
 */
class Mul : public OperatorNode {
  ...
  Eigen::MatrixXd derivative(size_t input_wrt_index) override {
    if (input_wrt_index) { // second input
      return this->getInput(0).transpose() * this->outer_derivative;
    } else { // first input
      return this->outer_derivative * this->getInput(1).transpose();
    }
  }
};
...
class EleWiseMul : public OperatorNode {
  ...
  Eigen::MatrixXd derivative(size_t input_wrt_index) override {
    if (input_wrt_index) { // second input
      return this->getInput(0).array() * this->outer_derivative.array();
    } else { // first input
      return this->getInput(1).array() * this->outer_derivative.array();
    }
  }
};
 ...
 *
 * NOTE: The graph will not find the partial derivative with respect to scalar
 * input(second input)
 *
...
class EleWiseDiv : public OperatorNode {
  ...
  Eigen::MatrixXd derivative(size_t input_wrt_index) override {
    if (input_wrt_index) { // second input
      return -(this->getInput(0).array() / this->getInput(1).array().pow(2)) *
             this->outer_derivative.array();
    } else { // first input
      return this->getInput(1).array().inverse() *
             this->outer_derivative.array();
    }
  }
};
...
class Div : public OperatorNode {
  ...
  Eigen::MatrixXd derivative(size_t input_wrt_index) override {
    if (input_wrt_index) { // second input
      ...
      return (1 / this->getInput(1)(0, 0)) * this->outer_derivative.array();
    }
  }
};
...
/**
 * Power operator: raise every elements of a matrix to the power of a scalar
 * value. a[i][j]^b TYPE: ELEMENT WISE
 *
 * DERIVATIVE:
 *      ∂a = 1
 *
...
class Pow : public OperatorNode {
  ...
  Eigen::MatrixXd derivative(size_t input_wrt_index) override {
    double exponent = this->getInput(1)(0, 0);
    return this->getInput(0).array().pow(exponent - 1) * exponent *
...
class Sqrt : public OperatorNode {
  ...
  Eigen::MatrixXd derivative(size_t input_wrt_index) override {
    return (1 / (2 * this->getInput().array().sqrt())) *
           this->outer_derivative.array();
  }
};
...
class Sin : public OperatorNode {
  ...
  Eigen::MatrixXd derivative(size_t input_wrt_index) override {
    return this->getInput().array().cos() * this->outer_derivative.array();
  }
};
...
class Cos : public OperatorNode {
  ...
  Eigen::MatrixXd derivative(size_t input_wrt_index) override {
    return -this->getInput().array().sin() * this->outer_derivative.array();
  }
};
...
class Sinh : public OperatorNode {
  ...
  Eigen::MatrixXd derivative(size_t input_wrt_index) override {
    return this->getInput().array().cosh() * this->outer_derivative.array();
  }
};
...
class Cosh : public OperatorNode {
  ...
  Eigen::MatrixXd derivative(size_t input_wrt_index) override {
    return this->getInput().array().sinh() * this->outer_derivative.array();
  }
};
...
class Exp : public OperatorNode {
  ...
  Eigen::MatrixXd derivative(size_t input_wrt_index) override {
    return this->getInput().array().exp() * this->outer_derivative.array();
  }
};
...
class Loge : public OperatorNode {
  ...
  Eigen::MatrixXd derivative(size_t input_wrt_index) override {
    return this->getInput().array().inverse() * this->outer_derivative.array();
  }
};
...
public:
  ...
  void compute() override {
    this->value = Eigen::MatrixXd::Constant(1, 1, this->getInput().sum());
  }
  Eigen::MatrixXd derivative(size_t input_wrt_index) override {
    return this->outer_derivative;
  }
...
class LeakyReLU : public OperatorNode {
  ...
  void compute() override {
    ...
    this->value = this->getInput().cwiseMax(leak_value * this->getInput());
  }
  Eigen::MatrixXd derivative(size_t input_wrt_index) override {
    double leak_value = this->getInput(1)(0, 0);
    if (leak_value == 0)
...
class Sigmoid : public OperatorNode {
  ...
  void compute() override {
    // 1 / (1 + exp(-x))
    this->value = (1 + (-this->getInput().array()).exp()).inverse();
  }
 ...
 *
 *          [ exp11sum - e1e1           e1e2                 e1e3     ]
 * <-jacobian [       e2e1          exp22sum - e2e2            e2e3     ] [ e3e1
 * e3e2          exp33sum - e3e3 ]
 *
...
class Softmax : public OperatorNode {
  ...
  Eigen::MatrixXd derivative(size_t input_wrt_index) override {
    ...
    Eigen::MatrixXd diagonal_exp_sum =
        Eigen ::MatrixXd ::Constant(exp.rows(), exp.rows(), 0);
    diagonal_exp_sum.diagonal() = exp * sum;

    Eigen::MatrixXd jacobian =
        (diagonal_exp_sum - (exp * exp.transpose())) / std::pow(sum, 2);
